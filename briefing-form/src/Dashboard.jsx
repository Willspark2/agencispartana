...
const TaskKanbanCard = ({ task, onClick, onDelete, onEdit }) => (
  <div onClick={onClick} className="bg-[#0a0a0a] border border-zinc-800 p-7 rounded-[2.2rem] hover:border-zinc-600 transition-all cursor-pointer group shadow-2xl relative overflow-hidden text-left">
     ...
     <div className="flex items-center justify-between pt-5 border-t border-zinc-900/50 text-left">
...
const InputHD = ({ label, ...props }) => (
  <div className="space-y-3">
    ...
    <input className="w-full bg-zinc-900 border border-zinc-800 rounded-[1.5rem] p-6 text-white focus:border-purple-500 focus:bg-black outline-none transition-all shadow-inner font-bold text-lg" {...props} />
  </div>
);
...
const TextAreaHD = ({ label, ...props }) => (
  <div className="space-y-3">
    <label className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.2em] ml-1">{label}</label>
    <textarea className="w-full bg-zinc-900 border border-zinc-800 rounded-[1.5rem] p-6 text-white focus:border-purple-500 focus:bg-black outline-none transition-all resize-none shadow-inner font-medium text-lg" {...props} />
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-zinc-400 font-sans flex overflow-hidden selection:bg-purple-500/30">
      ...
      <aside className={`${isSidebarOpen ? 'w-80' : 'w-24'} bg-[#080808] border-r border-zinc-900 transition-all duration-500 flex flex-col z-50 shadow-[20px_0_50px_rgba(0,0,0,0.8)]`}>
        ...
        <nav className="flex-1 px-6 space-y-12 overflow-y-auto custom-scrollbar text-left">
          <div className="space-y-3">
             <p className={`text-[10px] font-black text-zinc-800 uppercase tracking-[0.3em] mb-6 ${!isSidebarOpen && 'text-center'}`}>Workspace</p>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-black text-left">
        ...
        <div className="flex-1 overflow-y-auto p-12 custom-scrollbar z-30">
          ...
          {activeTab === 'projects' && (
            <div className="space-y-16 max-w-7xl animate-in fade-in duration-700">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
                  {projects.map(proj => (
                    <Card key={proj.id} className="p-12 bg-zinc-950/60 border-zinc-800/80 shadow-2xl relative group">
                       ...
                       <div className="flex justify-between items-start mb-12">
                          <div>
                            <h4 className="text-4xl font-black text-white uppercase italic tracking-tight mb-2 leading-none">{proj.name}</h4>
                            <div className="flex items-center gap-3">
                               <div className={`w-2 h-2 rounded-full ${proj.status === 'Ativo' ? 'bg-emerald-500' : 'bg-blue-500'} animate-pulse`} />
                               <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{proj.description}</span>
                            </div>
                          </div>
                       ...
                       <div className="space-y-4">
                          {tasks.filter(t => t.project === proj.name).map(t => (
                            <div key={t.id} className="flex items-center justify-between p-6 rounded-3xl bg-black border border-zinc-900 hover:border-purple-500/50 hover:bg-zinc-900/40 transition-all cursor-pointer group/item" onClick={() => { setSelectedTask(t); setShowDetails(true); }}>
                               <div className="flex items-center gap-5">
                                  <div className={`w-3 h-3 rounded-full ${t.status === 'in_progress' ? 'bg-blue-500' : 'bg-zinc-800'}`} />
                                  <span className="text-base font-bold text-zinc-400 uppercase group-hover/item:text-white transition-colors">{t.title}</span>
                               </div>
              ...
              {activeTab === 'dashboard' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                  ...
                  <div className="space-y-12">
                    ...
                    <Card className="p-10 border-purple-500/10 bg-purple-500/5 h-full flex flex-col min-h-[500px]">
                     ...
                     <div className="overflow-hidden rounded-[3rem] border border-zinc-900 bg-[#080808] shadow-2xl">
                        <table className="w-full text-left">
                          ...
                          <tbody className="divide-y divide-zinc-900">
                            <tr>
                              <td className="p-10 text-2xl font-black text-white uppercase italic tracking-tight leading-none">Cartão Black</td>
                              <td className="p-10 text-left"><input className="bg-transparent text-red-500 font-mono font-black text-4xl outline-none border-b-2 border-zinc-900 focus:border-red-600 transition-all w-full tracking-tighter" defaultValue="R$ 15.000,00" /></td>
          ...
          {activeTab === 'habits' && (
            <div className="space-y-16 animate-in fade-in duration-700 max-w-7xl mx-auto text-left">
      ...
      {showDetails && selectedTask && (
        <div className="fixed inset-0 bg-black/98 backdrop-blur-3xl z-[100] flex items-center justify-center p-8 lg:p-24 animate-in fade-in duration-500 text-left" onClick={() => setShowDetails(false)}>
           <div className="w-full max-w-6xl bg-[#050505] border border-zinc-800/50 rounded-[5rem] p-20 relative overflow-y-auto max-h-full" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-8 mb-10 text-left animate-in slide-in-from-left-4 duration-700">
                 <Badge variant={selectedTask.priority === 'CRÍTICA' ? 'critical' : 'high'}>{selectedTask.priority}</Badge>
                 <span className="text-[14px] font-black text-zinc-700 uppercase tracking-[0.8em] italic leading-none">Protocolo Spartana #{selectedTask.id}</span>
              </div>
              <h2 className="text-[120px] font-black text-white uppercase italic tracking-tighter leading-[0.8] mb-20 border-b border-zinc-900 pb-20 text-left">{selectedTask.title}</h2>
              ...
              <div className="mt-32 pt-16 border-t border-zinc-900 flex flex-col md:flex-row justify-between items-center gap-12 text-left">
                 ...
                 <div className="flex gap-6 pointer-events-auto text-left">
                    <button onClick={(e) => { e.stopPropagation(); setEditingTask(selectedTask); setNewTask({...selectedTask}); setShowModal(true); }} className="px-16 py-8 bg-zinc-100 text-black font-black rounded-[2.5rem] uppercase tracking-[0.2em] text-lg hover:bg-white transition-all shadow-xl active:scale-95">Editar Protocolo</button>
                    <button onClick={(e) => { e.stopPropagation(); handleDeleteTask(selectedTask.id); }} className="px-16 py-8 bg-red-950/20 text-red-600 border border-red-900/30 rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-lg hover:bg-red-600 hover:text-white transition-all active:scale-95">Abortar Missão</button>
                 </div>
              </div>
           </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-[110] flex items-center justify-center p-6 animate-in zoom-in-95 duration-500 text-left">
           <Card className="w-full max-w-5xl p-16 bg-[#050505] border-zinc-800 shadow-[0_0_150px_rgba(168,85,247,0.05)] text-left">
              <div className="flex justify-between items-center mb-16 border-b border-zinc-900 pb-10">
                 ...
                 <button onClick={closeModal} className="text-zinc-600 hover:text-white transition-all text-left"><X size={48} strokeWidth={1} /></button>
              </div>
              <form onSubmit={handleSaveTask} className="space-y-12 text-left">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16 text-left">
                   <div className="space-y-8 text-left">
                      ...
                      <div className="space-y-3">
                        <label className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.2em] ml-1">Projeto Relacionado</label>
                        <select className="w-full bg-zinc-900 border border-zinc-800 rounded-[1.5rem] p-6 text-white focus:border-purple-500 outline-none appearance-none font-bold text-lg" value={newTask.project} onChange={e => setNewTask({...newTask, project: e.target.value})}>
                ...
                <div className="flex justify-end gap-6 pt-12 border-t border-zinc-900 text-left">
                   ...
                   <button type="submit" className="px-20 py-8 bg-white text-black font-black rounded-[2rem] uppercase tracking-[0.2em] hover:scale-[1.02] transition-all text-xl">Confirmar Deploy</button>
                </div>
              </form>
           </Card>
        </div>
      ...
      {showProjectModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-[110] flex items-center justify-center p-6 animate-in zoom-in-95 duration-500 text-left">
           <Card className="w-full max-w-3xl p-16 bg-[#050505] border-zinc-800 text-left shadow-2xl">
              <div className="flex justify-between items-center mb-16 border-b border-zinc-900 pb-10">
                 <h2 className="text-5xl font-black uppercase italic tracking-tighter text-white">{editingProject ? 'Ajustar Contrato' : 'Novo Projeto'}</h2>
                 <button onClick={() => setShowProjectModal(false)} className="text-zinc-600 hover:text-white"><X size={48} strokeWidth={1} /></button>
              </div>
              <form onSubmit={handleSaveProject} className="space-y-12 text-left">
                <div className="space-y-8 text-left">
                  ...
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.2em] ml-1">Status do Contrato</label>
                    <select className="w-full bg-zinc-900 border border-zinc-800 rounded-[1.5rem] p-6 text-white focus:border-purple-500 outline-none appearance-none font-bold text-lg" value={newProject.status} onChange={e => setNewProject({...newProject, status: e.target.value})}>
                ...
                <div className="flex justify-end gap-6 pt-12 border-t border-zinc-900 text-left">
                   <button type="button" onClick={() => setShowProjectModal(false)} className="px-10 py-6 text-zinc-700 font-black uppercase tracking-[0.3em] hover:text-white transition-all">Cancelar</button>
                   <button type="submit" className="px-20 py-8 bg-white text-black font-black rounded-[2rem] uppercase tracking-[0.2em] hover:scale-[1.02] transition-all text-xl">Lançar Projeto</button>
                </div>
              </form>
           </Card>
        </div>
